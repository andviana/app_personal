...
from app.decorators import handle_permission_error
...
@login_required
@handle_permission_error('lists.index')
def delete(id):
...
@login_required
@handle_permission_error('lists.index', archived='true')
def reactivate(id):
...
def export_pdf(id):
    ...
    return PDFService.generate_list_pdf(id, current_user)
