...
def upgrade():
    ...
    if 'user' not in tables and 'User' not in tables:
    ...
    if 'shared_simple_lists' not in tables:
    ...
    lista_cols = [c['name'] for c in inspector.get_columns('lista')]
    with op.batch_alter_table('lista', schema=None) as batch_op:
        if 'owner_id' not in lista_cols:
    ...
    with op.batch_alter_table('tarefa', schema=None) as batch_op:
        if 'owner_id' not in tarefa_cols:
            batch_op.add_column(sa.Column('owner_id', sa.Integer(), nullable=True))
    ...
    bind.execute(
        sa.text("UPDATE tarefa SET owner_id = :aid, is_active = true WHERE owner_id IS NULL"),
        {"aid": anderson_id}
    )
    bind.execute(
        sa.text("UPDATE lista SET owner_id = :aid, is_active = true WHERE owner_id IS NULL"),
        {"aid": anderson_id}
    )
